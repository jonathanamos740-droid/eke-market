...
const getCached = (key, customDuration) => {
...
const lastGood = {};
const fetchWithFallback = async (key, fetchFn) => {
...
app.get('/api/coins', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('coins', async () => {
            const response = await (0, node_fetch_1.default)('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=100&page=1&sparkline=true&price_change_percentage=1h,24h,7d', { headers: { 'x-cg-demo-api-key': COINGECKO_API_KEY } });
            if (!response.ok)
                throw new Error(`CoinGecko failed: ${response.status} ${response.statusText}`);
...
const convertListCoinToDetail = (coin) => ({
    id: coin.id,
...
app.get('/api/coins/:id', async (req, res) => {
    ...
    try {
        const response = await (0, node_fetch_1.default)(`https://api.coingecko.com/api/v3/coins/${id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false`, { headers: { 'x-cg-demo-api-key': COINGECKO_API_KEY } });
        if (!response.ok) {
        ...
        if (coinsListCached && Array.isArray(coinsListCached)) {
            const foundCoin = coinsListCached.find((c) => c.id === id);
            if (foundCoin) {
...
app.get('/api/coins/:id/chart', async (req, res) => {
    ...
    try {
        const response = await (0, node_fetch_1.default)(`https://api.coingecko.com/api/v3/coins/${id}/market_chart?vs_currency=usd&days=${days}`, { headers: { 'x-cg-demo-api-key': COINGECKO_API_KEY } });
        if (!response.ok) {
            if (lastGood[cacheKey]) {
                return res.json({ success: true, data: lastGood[cacheKey], source: 'fallback' });
            }
...
app.get('/api/global', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('global', async () => {
            const response = await (0, node_fetch_1.default)('https://api.coingecko.com/api/v3/global');
            if (!response.ok)
                throw new Error('CoinGecko failed');
            return response.json();
        });
...
app.get('/api/trending', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('trending', async () => {
            const response = await (0, node_fetch_1.default)('https://api.coingecko.com/api/v3/search/trending', {
...
app.get('/api/price/:symbol', async (req, res) => {
    ...
    try {
        const { data } = await fetchWithFallback(`price_${symbol}`, async () => {
            const response = await (0, node_fetch_1.default)(`https://api.binance.com/api/v3/ticker/24hr?symbol=${symbol.toUpperCase()}USDT`);
            if (!response.ok)
...
app.get('/api/rates', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('rates', async () => {
            const response = await (0, node_fetch_1.default)('https://api.exchangerate-api.com/v4/latest/USD');
            if (!response.ok)
                throw new Error('Exchange rates failed');
            return response.json();
        });
        res.json({ success: true, data });
    }
...
app.get('/api/search', async (req, res) => {
    ...
    try {
        const { data } = await fetchWithFallback(`search_${query}`, async () => {
            const response = await (0, node_fetch_1.default)(`https://api.coingecko.com/api/v3/search?query=${query}`, { headers: { 'x-cg-demo-api-key': COINGECKO_API_KEY } });
            if (!response.ok)
...
app.get('/api/gainers-losers', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('gainers_losers', async () => {
            const response = await (0, node_fetch_1.default)('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&page=1&price_change_percentage=24h', { headers: { 'x-cg-demo-api-key': COINGECKO_API_KEY } });
            if (!response.ok)
                throw new Error('Gainers losers failed');
            return response.json();
        });
        ...
        res.json({
            success: true,
            data: {
                gainers: sorted.filter((c) => (c.price_change_percentage_24h || 0) > 0).slice(0, 10),
                losers: sorted.filter((c) => (c.price_change_percentage_24h || 0) < 0).slice(0, 10),
            }
        });
    }
    catch (err) {
...
app.get('/api/compare', async (req, res) => {
    ...
    try {
        const { data } = await fetchWithFallback(`compare_${ids}`, async () => {
            const response = await (0, node_fetch_1.default)(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${ids}&order=market_cap_desc&per_page=10&page=1&sparkline=true&price_change_percentage=1h,24h,7d`, { headers: { 'x-cg-demo-api-key': COINGECKO_API_KEY } });
            if (!response.ok)
...
app.get('/api/fear-greed', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('fear_greed', async () => {
            const response = await (0, node_fetch_1.default)('https://api.alternative.me/fng/?limit=1');
            if (!response.ok)
                throw new Error('Fear greed failed');
            return response.json();
        });
        res.json({ success: true, data });
    }
...
app.get('/api/prices', async (req, res) => {
    try {
        const { data } = await fetchWithFallback('binance_prices', async () => {
            const response = await (0, node_fetch_1.default)('https://api.binance.com/api/v3/ticker/24hr');
            if (!response.ok)
                throw new Error('Binance failed');
            return response.json();
        });
...
const warmCache = async () => {
    try {
        await (0, node_fetch_1.default)(`http://localhost:${PORT}/api/coins`);
        await (0, node_fetch_1.default)(`http://localhost:${PORT}/api/global`);
        await (0, node_fetch_1.default)(`http://localhost:${PORT}/api/trending`);
...
setInterval(async () => {
    try {
        await (0, node_fetch_1.default)(`${SELF_URL}/health`);
